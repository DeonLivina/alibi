"""Prior helpers for parameter sampling.

Re-exports the ``ml4gw.distributions`` (and ``torch.distributions``) classes that
the YAML configs reference by dotted path, and adds a ``Triangular`` distribution
that is occasionally handy for glitch/parameter priors.

The config loader resolves distributions via their fully-qualified ``func`` path
(e.g. ``ml4gw.distributions.PowerLaw``), so anything importable works. This module
simply provides a single, documented place to find the commonly used ones.
"""
...
# Re-export the distributions referenced from the YAML configs.
from ml4gw.distributions import PowerLaw, Sine, Cosine, DeltaFunction  # noqa: F401

__all__ = ["PowerLaw", "Sine", "Cosine", "DeltaFunction", "Uniform", "Triangular"]
...
class Triangular(Distribution):
    ...
    Inspired by
    https://github.com/tensorflow/probability/blob/master/tensorflow_probability/python/distributions/triangular.py
    """
    ...
    def sample(self, sample_shape=torch.Size()):
        ...
        with torch.no_grad():
            ...
            return torch.where(
                ...
                self.high
                - torch.sqrt((1.0 - samples) * interval_length * (self.high - self.peak)),
            )
